...
export class FindClientsComponent implements OnInit {
  ...
  confirmDeleteClient(id: number) {
    this._clientService.delete(id).subscribe({
      next: () => {
        this.messageAlert('Cliente deletado com sucesso!', true);
      },
      error: () => {
        this.messageAlert('Cliente não pode ser deletado!', false);
      }
    });
  }
  ...
  closeClientDetailsComponent() {
    ...
    this._sidebarService
      .getSidebar(SidebarNames.COMPONENT_CLIENT_DETAILS)
      .closeSidenav();
  }
  ...
  openSidebaCreateClient() {
    ...
    this._sidebarService
      .getSidebar(SidebarNames.COMPONENT_CLIENT_FORM)
      .openSidebar();
  }
